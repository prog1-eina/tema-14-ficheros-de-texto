 ...
 * 
 *       Puede ejecutarse en Windows, después de ser compilado,
 *       a través de la orden
 *           .\1-contar.exe
 *       y en Linux o macOS, a través de la orden
 *           ./1-contar
 *       y en ambos casos, desde Visual Studio Code,
 *       ejecutando la tarea "Ejecutar «contar»".
 *****************************************************************************/
 ...
 * Post: Si el fichero de nombre «nombreFichero» se puede leer, asigna a
 *       «nLineas» el número de líneas que contiene del fichero y a
 *       «nCaracteres», el número de caracteres del mismo y asigna «true» a 
 *       «lecturaOK». En caso contrario, asigna «false» a «lecturaOK» e informa
 ...
 *       accesible para su lectura.
 * Post: Si el fichero de nombre «nombreFichero» se puede leer, asigna a
 *       «nLineas» el número de líneas que contiene el fichero y a
 *       «nCaracteres» el número de caracteres del mismo. Asigna «true» a
 *       «lecturaOk». En caso contrario, asigna «false» a «lecturaOk» e
 *       informa del error ocurrido por «cerr».
...
void contabilizar2(const string nombreFichero,
    ...
    if (f.is_open()) {
        ...
        while (f.get(c)) {
            // Mientras el último intento de lectura fue correcto
            nCaracteres++;              // Cuenta el último carácter leído.
            if (c == '\n') {
                // Si el último carácter leído es un cambio de línea...
 ...
 * Post: Realiza una prueba de la función «contabilizar», invocándola con el
...
int main() {
    ...
    if (lecturaOk) {
        cout << '\"' << NOMBRE_FICHERO << "\" tiene " << lineas
             << " líneas y "  << caracteres << " caracteres." << endl;
        return 0;
    } else {
        return 1;
    }
